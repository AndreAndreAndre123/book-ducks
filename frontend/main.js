...
let bookDucksReload = document.querySelector(".book-ducks").addEventListener("click", () =>{
    console.log("Reloaded book ducks.")
    location.reload();
})
...
addBookHeader.addEventListener("click", () =>{
    allBooksContainer.classList.add("hidden");
    document.querySelector(".add-new-book ").classList.remove("hidden");
...
// används ej för tillfället (instanse istället för axios)
...
addNewBook.addEventListener('submit', async event => {
...
let printBooksBtn =  async (e) =>{
    ...
    let book = books.map((e)=>{
        ...
        <p class="book-genre-style text"> ${genre ? "genre: " + genre.replaceAll(",", " / ") : "no genre"}</p>
...
profileStatus.addEventListener("click", () => {
    ...
    registerForm.classList.add("hidden");
    logInForm.classList.add("hidden");
    document.querySelector(".add-new-book ").classList.add("hidden");
    allBooksContainer.classList.add("hidden")
})
